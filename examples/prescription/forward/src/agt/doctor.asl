...
+!sendPrescribePharmacist : symptom(Symptom)[source(Patient)] & treatment(Symptom,Drug)
	<- .print("Sending prescription to pharmacist...");
	   //.send(pharmacist,tell,prescription(Drug,Patient)).
	   .

+!sendPrescribePatient : symptom(Symptom)[source(Patient)] & treatment(Symptom,Drug)
	<- .print("Sending prescription to patient...");
	   .send(Patient,tell,prescription(Drug,Patient)).
