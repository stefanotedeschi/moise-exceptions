...
+obligation(Ag,_,done(sls_sch,unsure_no,Ag),_)
	...
	<- println("I'm sure about the solution!");
...
+obligation(Ag,_,done(sls_sch,unsure_yes,Ag),_)
	 : .my_name(Ag) &
	   unsure(yes) &
	   scheme(sls_sch,_,SchId)
	<- println("I'm unsure! Better ask developer");
...
+obligation(Ag,_,done(sls_sch,result_fix_in_next_release,Ag),_)
	 : .my_name(Ag) &
	   result(next_release) &
	   scheme(sls_sch,_,SchId)
	<- println("Issue must be fixed in next release");
...
+feedback(F)
	...
	<- println("Feedback received from second level support!");
...
+obligation(Ag,_,done(sls_sch,root_2nd_level_support,Ag),_)
	 : .my_name(Ag) &
	   scheme(sls_sch,_,SchId)
	<- println("*** SCHEME SECOND LEVEL SUPPORT COMPLETED ***");
...
+obligation(Ag,_,done(sls_sch,handle_second_level_exception,Ag),_)
	...
	<- println("Handling second level exception... Cannot achieve ask_2nd_level_support");
...
+obligation(Ag,_,done(fls_sch,raise_first_level_exception,Ag),_)
	 : .my_name(Ag) &
	   scheme(fls_sch,_,SlsSchId) &
	   exceptionThrown(sls_sch,second_level_exception,_) &
	   exceptionArgument(sls_sch,second_level_exception,warrantyStatus(no))
	<- println("Raising first level exception! Product out of warranty");
	   throwException(first_level_exception,[warrantyStatus(no)])[artifact_id(FlsSchId)];
