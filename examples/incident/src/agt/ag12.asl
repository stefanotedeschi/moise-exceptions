+obligation(Ag,_,done(sls_sch,ask_developer,Ag),_)
	...
	<- println("Asking support to developer...");
...
+result(R)
	...
	<- println("Feedback received from developer!");
...
+obligation(Ag,_,done(dev_sch,root_developer,Ag),_)
	...
	<- println("*** SCHEME DEVELOPER COMPLETED ***");
...
+obligation(Ag,_,done(dev_sch,handle_developer_exception,Ag),_)
	...
	<- println("Handling developer exception... Cannot achieve ask_developer");
...
+obligation(Ag,_,done(sls_sch,raise_second_level_exception,Ag),_)
	...
	<- println("Raising second level exception! Product out of warranty");
