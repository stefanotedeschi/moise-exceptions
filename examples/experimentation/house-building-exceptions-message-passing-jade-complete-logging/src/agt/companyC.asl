...
+!windows_fitted[scheme(S)]
   ...
   <- .print("Notifying weeks of delay");
      .random([0,1],N);
      if(N == 0) {
         logException(windows_delay_exception)[artifact_id(LogArtId)];
...
+!exterior_painted
    : play(HouseOwner,house_owner,hsh_group) &
      ...
      loggerArtifact(LogArtId)
   <- logException(exterior_paint_exception)[artifact_id(LogArtId)];
...
+!interior_painted[scheme(S)]
    : play(HouseOwner,house_owner,hsh_group) &
      ...
      loggerArtifact(LogArtId)
   <- logException(interior_paint_exception)[artifact_id(LogArtId)];
