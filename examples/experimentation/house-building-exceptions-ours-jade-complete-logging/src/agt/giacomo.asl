...
+obligation(Ag,Norm,What,Deadline)[artifact_id(ArtId)]
   ...
   <- //.print("********** OBLIGATION FOR ", Goal);
      logObligation[artifact_id(LogArtId)].

+exceptionRaised(bhsch,E,_)[artifact_id(ArtId)]
   <- //.print("********** RAISED ", E);
      logException(E).
...
+!choose_new_color
   ...
   <- //print("CHOOSE NEW COLOR EXTERIOR");
      logObligation[artifact_id(LogArtId)];
...
+!choose_new_color
   ...
   <- //print("CHOOSE NEW COLOR INTERIOR");
      logObligation[artifact_id(LogArtId)];
