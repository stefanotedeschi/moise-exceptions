...
/**
 * Artifact to manage a scheme instance. <br/>
 * <br/>
 *
 ...
 * <ul>
 * <li>commitment(ag,mission,sch): agent ag is committed to the mission in the
 * scheme (we have as many obs prop as commitments).</br>
 * e.g. <code>commitment(bob,mission1,s1)</code>
 * <li>groups: a list of groups responsible for the scheme.</br>
 * e.g. <code>groups([g1])</code>
 * <li>goalState(schId, goal, list of committed agents, list of agents that
 * performed the goal, state); where states are: waiting, enabled,
 * satisfied).</br>
 * e.g. <code>goalState(s1,g5,[alice,bob],[alice],satisfied)</code>
 * <li>specification: the specification of the scheme in the OS (a prolog like
 * representation).
 * <li>obligation(ag,reason,goal,deadline): current active obligations.</br>
 * e.g.
 * <code>obligation(bob,ngoal(s1,mission1,g5),done(s1,bid,bob),1475417322254)</code>
 * <li>permission(ag,reason,goal,deadline): current active permission.</br>
 *
 * <li>goalArgument(schemeId, goalId, argId, value): value of goals' arguments,
 * defined by the operation setArgumentValue</br>
 * e.g. <code>goalArgument(sch1, winner, "W", "Bob")</code>
 * </ul>
 *
 * <b>Signals</b> (obligation o has the form: obligation(to whom, maintenance
 * condition, what, deadline)):
 * <ul>
 * <li>oblCreated(o): the obligation <i>o</i> is created.
 * <li>oblFulfilled(o): the obligation <i>o</i> is fulfilled
 * <li>oblUnfulfilled(o): the obligation <i>o</i> is unfulfilled (e.g. by
 * timeout).
 * <li>oblInactive(o): the obligation <i>o</i> is inactive (e.g. its maintenance
 * condition does not hold anymore).</br>
 * e.g. <code>o = obligation(Ag,_,done(Sch,bid,Ag), TTF)</code> if the
 * <code>bid</code> is a performance goal and
 * <code>o = obligation(Ag,_,satisfied(Sch,bid), TTF)</code> if the
 * <code>bid</code> is an achievement goal.
 * <li>normFailure(f): the failure <i>f</i> has happened (e.g. due some
 * regimentation).</br>
 * e.g. <code>f = fail(mission_permission(Ag,M,Sch))</code>. The f comes from
...
public class SchemeBoard extends OrgArt {
		...
		for (Player p : orgState.getPlayers()) {
			if (orgState.removePlayer(agName, p.getTarget())) {
				try {
					...
					removeObsPropertyByTemplate(obsPropCommitment, new JasonTermWrapper(agName),
							new JasonTermWrapper(p.getTarget()), this.getId().getName());
					// updateMonitorScheme();
					updateGuiOE();
	...
	/**
	 * The agent executing this operation tries to commit to a mission in the
	 * scheme.
	 *
	 * <p>
	 * Verifications:
	 ...
	 * <li>mission permission (if the agent plays a role that permits it to commit
	 * to the mission)</li>
	 * </ul>
	 *
	 * @param mission the mission being committed to
	 * @throws NormativeFailureException the failure produced if the adoption breaks
	 *                                   some regimentation
	 * @throws CartagoException          some cartago problem
	 */
	@OPERATION
	public void commitMission(String mission) throws CartagoException {
	...
	protected void commitMission(final String ag, final String mission) throws CartagoException {
		...
		ora4masOperationTemplate(new Operation() {
			public void exec() throws NormativeFailureException, Exception {
				...
				defineObsProperty(obsPropCommitment, new JasonTermWrapper(ag), new JasonTermWrapper(mission),
	...
	/**
	 * The agent executing this operation tries to leave/remove its mission in the
	 * scheme
	 *
	 * <p>
	 * Verifications:
	 ...
	 * <li>the mission's goals have to be satisfied (otherwise the agent is obliged
     * to commit again to the mission)</li>
     * </ul>
     *
     * @param mission the mission being removed
     * @throws NormativeFailureException the failure produced if the remove breaks
     *                                   some regimentation
     * @throws CartagoException          some cartago problem
     * @throws MoiseException            some moise inconsistency (the agent is not
     *                                   committed to the mission)
     */
    @OPERATION
    public void leaveMission(final String mission) throws CartagoException, MoiseException {
        ora4masOperationTemplate(new Operation() {
            public void exec() throws NormativeFailureException, Exception {
                if (orgState.removePlayer(getOpUserName(), mission)) {
                    nengine.verifyNorms();

                    removeObsPropertyByTemplate(obsPropCommitment, new JasonTermWrapper(getOpUserName()),
                            new JasonTermWrapper(mission), new JasonTermWrapper(SchemeBoard.this.getId().getName()));
                }
            }
    ...
    @OPERATION
    public void goalAchieved(String goal) throws CartagoException {
     ...
     */
    @OPERATION
    public void setArgumentValue(final String goal, final String var, final Object value) throws CartagoException {
        ora4masOperationTemplate(new Operation() {
            public void exec() throws NormativeFailureException, Exception {
                Object pvl = value;
                ...
                updateGoalArgsObsProp();
                /*
                 * defineObsProperty("goalArgument",
                 * ASSyntax.createAtom(SchemeBoard.this.getId().getName()), new Atom(goal),
                 * ASSyntax.createString(var), pvl);
                 */
            }
    ...
    /**
     * The agent executing this operation reset some goal. It becomes not achieved,
     * also goals that depends on it or sub-goals are set as unachieved
     * 
     * @param goal The goal to be reset
     * @throws ParseException 
     */
    @OPERATION
    public void resetGoal(final String goal) throws CartagoException, ParseException {
...
//        nplp parser = new nplp(new StringReader("scheme_id(S) & failed(S,g2)"));
//        parser.setDFP(this);
//        LogicalFormula formula = (LogicalFormula)parser.log_expr();
//        System.out.println("Formula: "+formula+" = "+nengine.holds(formula));
//        
        ...
        ora4masOperationTemplate(new Operation() {
            public void exec() throws NormativeFailureException, Exception {
                ...
                nengine.verifyNorms();
                getSchState().clearExPlayers(); // and an agent quits a mission accomplished, that was ok, the reset
                                                // goal will turn this mission unaccomplished and produces a norm
                                                // failure. so we remove the ex players here
                updateGoalStateObsProp();
                updateExceptionsObsProp();
            }
    ...
    @OPERATION
    public void getState(OpFeedbackParam<Scheme> s) {
        s.set(getSchState());
    }
     ...
     *
     * @param cmd, possible values (as strings): commitMission(<agent>,<mission>),
    ...
    @OPERATION
    public void addMission(String missionId) throws MoiseException, ParseException {
        if (!isUserAllowed()) {
            failed("Error: agent '" + getOpUserName() + "' is not allowed to add missions", "reason",
                    new JasonTermWrapper("not_allowed_to_start(addMission)"));
        } else {
    ...
    public void setCardinality(String element, String id, int min, int max) throws MoiseException, ParseException {
        if (!isUserAllowed()) {
            failed("Error: agent '" + getOpUserName() + "' is not allowed to set cardinality", "reason",
                    new JasonTermWrapper("not_allowed_to_start(setCardinality)"));
        } else {
    ...
    @LINK
    public void interactionCommand(String cmd) throws CartagoException, jason.asSyntax.parser.ParseException {
        Literal lCmd = ASSyntax.parseLiteral(cmd);
        ...
        ora4masOperationTemplate(new Operation() {
            public void exec() throws NormativeFailureException, Exception {
                ...
                if (newLink) {
                    ...
                    // ArtifactId aid = makeArtifact(nbId, getNormativeBoardClass(), new
                    // ArtifactConfig() );
        ...
        for (Pair<String, String> garg : getSchState().getGoalsArgs().keySet()) {
            try {
                defineObsProperty("goalArgument", ASSyntax.createAtom(SchemeBoard.this.getId().getName()),
                        new Atom(garg.getLeft()), ASSyntax.createString(garg.getRight()),
    ...
    protected void updateExceptionsObsProp() {
        ...
        while (i.hasNext()) {
            ...
            if (!nengine.holds(new NPLLiteral(l, this))) {
                ...
                while (iArgs.hasNext()) {
                    ObsProperty arg = iArgs.next();
                    if (arg.getValue(1).toString().equals(op.getValue(1).toString())) {
                        iArgs.remove();
                        removeObsPropertyByTemplate(arg.getName(), arg.getValues());
                    }
                }
                i.remove();
                removeObsPropertyByTemplate(op.getName(), op.getValues());
    ...
    /*
     * private void updateMonitorScheme() throws CartagoException { if
     * (monitorSchArt != null) { execLinkedOp(monitorSchArt,
     * "updateMonitoredScheme", orgState); } }
     */

    /*
     * public static List<String> computeAccomplisedMissions(String schId,
     * Collection<Mission> missions, NPLInterpreter nengine) { Atom aSch = new
     * Atom(schId); List<String> as = new ArrayList<String>(); for (Mission m:
     * missions) { boolean all = true; for (Goal g: m.getGoals()) {
     * //System.out.println(m+" "+g); Atom aGoal = new Atom(g.getId()); if
     * (!nengine.holds(ASSyntax.createLiteral("satisfied", aSch, aGoal))) { all =
     * false; //System.out.println("not ok for "+aSch+" "+aGoal); break; } } if
     * (all) as.add(m.getId()); } return as; }
     */
    ...
    List<Literal> getGoalStates() {
        ...
        for (Goal g : spec.getGoals()) {
            ...
            /*
             * if (g.hasArguments()) { for (String arg: g.getArguments().keySet()) { String
             * value = getSchState().getGoalArgValue(g.getId(), arg); if (value == null) {
             * lGoal.addTerm(new VarTerm(arg)); } else { try {
             * lGoal.addTerm(ASSyntax.parseTerm(value)); } catch
             * (jason.asSyntax.parser.ParseException e) { lGoal.addTerm(new
             * StringTermImpl(value)); } } } }
             */
            ...
            // create the literal
            Literal lGoalSt = ASSyntax.createLiteral(piGoalState.getFunctor(), tSch, lGoal,
    ...
    public String getAsDot() {
        ...
        if (!spec.getId().equals("untyped"))
            type = ": " + spec.getId();
        so.append("digraph " + getId() + " {ordering=out label=\"" + getId() + type
				+ "\" labelloc=t labeljust=r fontname=\"Italic\" \n");
    ...
    /*
     * @LINK void updateMonitoredScheme(Scheme monitoredSch) throws
     * NormativeFailureException, CartagoException { // TODO
     * //model.setMonitoredSch(monitoredSch);
     * //nengine.setDynamicFacts(oe2np.transform(model)); //nengine.verifyNorms();
     * //updateGuiOE(); }
     */
}
