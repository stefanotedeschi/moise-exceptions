...
import moise.os.fs.RecoveryStrategy;
...
public class os2nopl {
    ...
    public static final String[] NOP_GR_PROPS = new String[] { PROP_RoleInGroup, PROP_RoleCardinality,
            PROP_RoleCompatibility, PROP_WellFormedResponsible, PROP_SubgroupInGroup, PROP_SubgroupCardinality };
    ...
    static {
        ...
        condCode.put(PROP_RoleCompatibility,
                "play(Agt,R1,Gr) & play(Agt,R2,Gr) & group_id(Gr) & R1 < R2 & not fcompatible(R1,R2,gr_inst)"); // note
                                                                                                                // that
                                                                                                                // is
                                                                                                                // have
                                                                                                                // to be
        ...
        condCode.put(PROP_SubgroupCardinality,
                "group_id(Gr) & subgroup_cardinality(SG,_,SGMax) & .count(subgroup(_,SG,Gr),SGP) & SGP > SGMax");

        condCode.put(PROP_WellFormedResponsible, "responsible(Gr,S) & not well_formed(Gr)"); // not monitor_scheme(S) &
        condCode.put(PROP_MissionPermission,
                "committed(Agt,M,S) & not (mission_role(M,R) & responsible(Gr,S) & fplay(Agt,R,Gr))");
    ...
    public static String transform(Group gr) {
        ...
        for (Role r : roles) {
            ...
            np.append(sep + "      rplayers(" + r.getId() + ",G," + var + ") & " + var + " >= " + c.getMin() + " & "
                    + var + " <= " + c.getMax());
            sep = " &\n";
        }
        for (Group sg : gr.getSubGroups()) {
            ...
            np.append(sep + "      .count(subgroup(_," + sg.getId() + ",G)," + var + ") & " + var + " >= " + c.getMin()
                    + " & " + var + " <= " + c.getMax()); // +" & subgroup_well_formed("+sg.getId()+")");
            sep = " &\n";
        }
        np.append(sep
...
//                missionHandler += "   mission_handler("+m.getId()+","+t.getId()+").\n";
//                if(t.getGoal() != null) {
...
//        np.append(exception);
//        np.append(handler);
//        np.append(missionException);
//        np.append(exceptionGoal);
//        np.append(missionHandler);
//        np.append(handlerGoal);
        ...
        for (Mission m : sch.getMissions()) {
            ...
            np.append(sep + "      (mission_accomplished(S," + m.getId() + ") | not mission_accomplished(S," + m.getId()
                    + ") & mplayers(" + m.getId() + ",S," + var + ") & " + var + " >= " + c.getMin() + " & " + var
                    + " <= " + c.getMax() + ")");
            sep = " &\n";
        }
        np.append(".\n");
        ...
        np.append(
                "   enabled(S,G) :- goal(_, G,  dep(or,PCG), _, NP, _) & not released(_,G) & not exception_goal(_,G) & not handler_goal(_,G) & NP \\== 0 & (any_satisfied(S,PCG) | all_released(S,PCG)).\n");
        np.append(
                "   enabled(S,G) :- goal(_, G, dep(and,PCG), _, NP, _) & not released(_,G) & not exception_goal(_,G) & not handler_goal(_,G) & NP \\== 0 & all_satisfied_released(S,PCG).\n\n");

        np.append("   super_satisfied(S,G) :- super_goal(SG,G) & satisfied(S,SG).\n");
        
        np.append("   fault(O) :- O = obligation(A,enabled(S,G),What,D) & active(O) & ((goal(_,G,_,achievement,_,_) & What = satisfied(S,G)) | (goal(_,G,_,performance,_,_) & What = done(S,G,A))) & failed(S,G).\n\n");
        np.append("   normal(O) :- O = obligation(A,enabled(S,G),What,D) & active(O) & ((goal(_,G,_,achievement,_,_) & What = satisfied(S,G)) | (goal(_,G,_,performance,_,_) & What = done(S,G,A))) & not failed(S,G).\n\n");
        ...
        if (!isSB) {
            ...
            for (Norm nrm : sch.getFS().getOS().getNS().getNorms()) {
                if (sch.getMissions().contains(nrm.getMission())) {
                    np.append(generateNormEntry(nrm, sch.getMissionCardinality(nrm.getMission())));
                }
            }
        }
        ...
        if (isSB) {
            ...
            // TODO: implement location as annot for What. create an internal action to add
            // this annot?
            // np.append(" ((goal(_,G,_,_,_,_)[location(L)] & WhatL = What[location(L)]) |
...
//            np.append("        -> obligation(A,enabled(S,TG),What,`now` + D).\n\n");
    ...
    public static String generateNormEntry(Norm nrm, Cardinality card) {
        ...
        String extraCond = "not mission_accomplished(S," + m
                + ") // if all mission's goals are satisfied, the agent is not obliged to commit to the mission";
    ...
    public static String header(MoiseElement ele) {
        ...
        np.append("    the organisation specification '" + ele.getId() + "'\n    on "
                + new SimpleDateFormat("MMMM dd, yyyy - HH:mm:ss").format(new Date()) + "\n\n");
